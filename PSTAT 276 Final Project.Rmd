...
Next, we will simulate the next 1-year MSFT prices by weeks using our StockPath function from Problem 2. This simulation is done using standard Monte Carlo method. For more information, see the description below Problem 2 chunk.
...
First, we will use the American Put function from problem 4 to find the Monte-Carlo Machine-Learning Adjusted price. This function employs a neural net and recursive simulation at each time node to estimate the best price for the American Option. For more information on how the function works, see the explanation following Problem 4 chunk above. 
...
The result above preforms okay, about $4 above the current equivalent option price. We would hope to get a price a little closer to the current option price. We are unsure with the reason of the deviation in prices but we think this price may be more 'randomly' computed.
