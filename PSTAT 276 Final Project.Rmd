...
AmeOptPrice <- function(C,t, sigma, r, k ,delta){
  ...
  for (i in 1:nrow(C)){ 
    ...
    Sim_St <- StockPath(200, sigma, Second_to_last[i], dt, np=1, r, delta)
  ...
  for (j in seq(from=ncol(holding_value)-1, to=2, by=-1)){ #iterate from second to last column all the way to beginning
    ...
    for(k in 1:nrow(C)){ #iterate each row
      New_Sim_St <- StockPath(200, sigma, 
  ...
  for (z in 1:nrow(early_price)){
    ...
    if(length(exercise_indexes)==0){
      ...
      Option_Price[z] <-
        option_value[z,exercise_indexes[1]]*exp(-r*exercise_indexes[1]*dt)
...
## We first assign values to last column of option values and holding values with payoffs. After dealing with last column, we move forward to second to last column. We iterate through each row in our dataset and use every value to perform simulations based on StockPath function. After that, we can make comparsion between holding values and payoffs to get option values at second to last column. 

## Next, we work backwards from the second to last column to beginning. We build another for-loop to make iteration. In this part, we build a neural network model with changing stock prices as predictor and changing option values as response. We also generate new simulations based on new current stock prices. Then, we make predictions based on neural network model and get probabilites which determine our prediction scores. By multiplying probabilites and current stock prices, we can get estimated prices which can be used to find holding values by taking average. And we can get all values for option values by comparing holding values and payoffs.
...
MSFT <- read.csv(file = '/Users/lingyuzhou/Desktop/MSFT.csv', 
                   header = T, sep = ",")
